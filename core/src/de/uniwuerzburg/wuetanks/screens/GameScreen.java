...
import de.uniwuerzburg.wuetanks.entity.Direction;
import de.uniwuerzburg.wuetanks.entity.Entity;
import de.uniwuerzburg.wuetanks.entity.Obstacle;
...
public class GameScreen implements Screen {
	...
	private int fontSize = Wuetanks.getInstance().getPreferences().getInteger("game_font_size", 14);
	...
	private int spawnOffset = Wuetanks.getInstance().getPreferences().getInteger("spawn_offset", 20);
	private Music music;
	private float time;
	...
	private void setUpGraphics() {
		...
		generator = new FreeTypeFontGenerator(Gdx.files
				.internal(Wuetanks.getInstance().getPreferences().getString("game_font", "fonts/Vera.ttf")));
	...
	private void loadMusic() {
		music = Gdx.audio.newMusic(Gdx.files
				.internal(Wuetanks.getInstance().getPreferences().getString("background_music", "music.mp3")));
		// music.play();
	...
	public void render(float delta) {
		Gdx.graphics.setTitle(Wuetanks.getInstance().getPreferences().getString("title", "Wuetanks"));
		Gdx.gl.glClearColor(0, 0, 0, 1);
		Gdx.gl.glClear(GL20.GL_COLOR_BUFFER_BIT);

		time -= delta;
		if (time <= 0) {
			Wuetanks.getInstance().showEnd(players);
		}
	...
	private void loadMap() {
		...
		try {
			...
			tiledMap = new TmxMapLoader()
					.load(Wuetanks.getInstance().getPreferences().getString("default_map", "maps/default.tmx"));
		}
