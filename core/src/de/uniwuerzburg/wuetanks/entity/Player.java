...
public class Player extends Entity {
	...
	public Player(Tanks tank, int key_up, int key_down, int key_left, int key_right, int key_shoot) {
		...
		width = Wuetanks.getInstance().getPreferences().getInteger("player_width", 40);
		height = Wuetanks.getInstance().getPreferences().getInteger("player_height", 40);

		collisionRectangleWidth = width;
		collisionRectangleHeight = height;

		movingSpeed = Wuetanks.getInstance().getPreferences().getInteger("player_speed", 150);

		gunSprite = Wuetanks.getInstance().getTextureAtlas().createSprite("barrel" + tank.getName());
		gunSprite.setSize(Wuetanks.getInstance().getPreferences().getInteger("gun_width", 10),
				Wuetanks.getInstance().getPreferences().getInteger("gun_height", 40));
	...
	public void update() {
		...
		if (currentHitpoints == 0.f) {
			...
			if (position.x + speed.x * Gdx.graphics.getDeltaTime() + width > Wuetanks.getInstance().getGame()
					.getWidth()) {
				position.x = Wuetanks.getInstance().getGame().getWidth() - width;
				speed.x = 0;
			}
			...
			if (position.y + speed.y * Gdx.graphics.getDeltaTime() + height > Wuetanks.getInstance().getGame()
					.getHeight()) {
				position.y = Wuetanks.getInstance().getGame().getHeight() - height;
				speed.y = 0;
			}
	...
	public Vector2 getSpawnPoint() {
		float gameWidth = Wuetanks.getInstance().getGame().getWidth();
		float gameHeight = Wuetanks.getInstance().getGame().getHeight();

		int spawnOffset = Wuetanks.getInstance().getPreferences().getInteger("spawn_offset", 20);
		...
		for (Player p : Wuetanks.getInstance().getGame().getPlayers()) {
			if (p != this) {
	...
	public void createBullet() {
		...
		Wuetanks.getInstance().getGame().getEntities().add(bullet);
