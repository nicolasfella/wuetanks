...
public enum Tanks {
	...
	private Tanks(String name, float maxHitpoints, float damage, float armor, float reloadTime) {
		...
		shotSound = Wuetanks.getInstance().getAudio().newSound(Wuetanks.getInstance().getFiles()
				.internal(Wuetanks.getInstance().getPreferences().getString("shot_sound", "shot.wav")));
	}
